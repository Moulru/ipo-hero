...
- **릴리스 빌드**: `gradlew bundleRelease` → 서명 AAB(현재 versionCode 11·versionName 1.0.0·R8 minify on). 산출물=`GooglePlay 산출물/ipohero-v1.0.0-vc11.aab`(+mapping). (프로덕션 첫 제출까지 versionName 1.0.0 유지, vc만 증가.) 키=`android/app/release.keystore`+`keystore.properties`(gitignore, **백업 필수**, pw=`../_myToken_.md` 참고). **android/는 gitignore → 재생성 시** 서명·versionName + targetSdk/compileSdk 36(`variables.gradle`)·AGP 8.9.1(`build.gradle`)·Gradle 8.11.1(`wrapper`)·minifyEnabled true+`proguard-rules.pro`(Capacitor 보호 규칙) 재적용 필수. R8 mapping=`build/outputs/mapping/release/mapping.txt`(Play 업로드용).
...
**증권사별 배정 비교**(증권사별 실시간 경쟁률 소스 미확보 → 보류) · **Play Store 출시 진행 중**(비공개 테스트=v1.0.0/vc3 진행 중. **프로덕션용 v1.0.0/vc11 빌드 완료**[정보코어 v1.1 + 탭분리(정보/모의청약) + '모의청약' 리네이밍 + edge-to-edge safe-area + 군더더기 문구 정리 + 청약일정 짬뽕범위 수정(pickSchedule) + '최근 상장' 평균통계→최근5건 목록 전환(평균왜곡 해소)]=`GooglePlay 산출물/ipohero-v1.0.0-vc11.aab`, 14일 충족 후 제출 예정. 가이드·산출물=`GooglePlay 산출물/`, 방침=PRIVACY.md. ⚠️개인계정은 프로덕션 전 12명·14일 비공개 테스트 의무) · Pretendard는 이미 번들 · 시즌 랭킹(백엔드)
